	...
	public void fixFingerTable() {
		...
		try {
			...
			for (int i = 0; i < bitSize ; i++) {
				...
				BigInteger nextId = new BigInteger("2").pow(i);

				// then: use chordnode to find the successor of k. (i.e., succnode = chordnode.findSuccessor(k))

				BigInteger currentId = chordnode.getNodeID().add(nextId).mod(mod);
				try {
					successNode = chordnode.findSuccessor(currentId);
				} catch (RemoteException e) {
					e.printStackTrace();
				}
				...
				if(successNode != null) {
					try {
						fingerTable.set(i, successNode);
					} catch (IndexOutOfBoundsException e) {
						fingerTable.add(i, successNode);
					}
				}
			}
